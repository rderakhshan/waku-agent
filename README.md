...
and the per-run scores. Nothing else needs it: skip it and the department still
runs. It is **on by default** once configured; `IRINA_LAMINAR=off` turns it off.

Setup is four steps — run the stack, serve it under `/laminar`, create a project
and paste the key, then read the traces and the scores. The exact commands, the
compose override, and what does and does not get recorded are in
**[docs/laminar.md](docs/laminar.md)**.
